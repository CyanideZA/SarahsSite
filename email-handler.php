...
file_put_contents('order_log.txt', print_r($input, true), FILE_APPEND);
...
if (!filter_var($input['to'], FILTER_VALIDATE_EMAIL)) {
    http_response_code(400);
    die(json_encode(['error' => 'Invalid customer email']));
}

if (!filter_var($input['customer_email'], FILTER_VALIDATE_EMAIL)) {
    http_response_code(400);
    die(json_encode(['error' => 'Invalid customer email in order data']));
}
